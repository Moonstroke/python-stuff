...
def footer():
    ...
    r += '<td>' + x + '</td>'
    r = '<footer>\n'
    r += sub_blocks(img, ol, p, table, ul)
    return r
...
def li():
    ...
    if len(text):
        ...
        r += sub_blocks(a, h, img, ol, p, table, ul)
        r += '</li>\n'
    return r
...
_all = (header, article, section, aside, footer, a, h, img, li, ol, p, pre, table, ul)

def sub_blocks(*authorized_blocks):
...
nb_articles = 0
nb_sections = 0
html_body = sub_blocks()

if not has_header:
    set_header = get('Page has no header. Set one? [y/N]', True).lower()
    if not set_header in 'n':
        html_body = header() + html_body()
if not has_header:
    set_footer = get('Page has no footer. Set one? [y/N]', True).lower()
    if not set_footer in 'n':
        html_body += footer()
html_body = '<body>\n' + html_body + '</body>\n'
html_body += '</html>\n'
